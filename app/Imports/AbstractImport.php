        ...
        foreach($collection as $key => $row) {
            if ($key == 0) {
                if (count($row) < count($this->names)) {
                    ...
                    foreach ($this->names as $index => $keyName) {
                        array_push($this->keys, $index);
            ...
            //dd(count($this->keys));
            foreach($row as $name => $row_key) {
                if ($name < count($this->keys)) {
                    if ($row[$name]) $anyExist = true;
                    //$anyExist = true;
                    $this_name = trim($row_key);
                    $data[$this->keys[$name]] = $this_name !== '' ? $this_name : null;
                }
            }
