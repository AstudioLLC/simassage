...
class FileManager
{
    public function uploadImage($key, $path, $sizes, $delete = false, $item = false)
    {
        ...
        if (!empty($image)) {
            if (in_array($image->extension(), ['jpeg','webp','jpg', 'png', 'gif']) && $image->isValid()) {
                $ext = $image->extension();
                if ($ext == 'jpeg') $ext = 'jpg';
                $ext = 'webp';
    ...
    public function uploadOriginalImage($key, $path, $delete = false)
    {
        ...
        if (!empty($image)) {
            if (in_array($image->extension(), ['jpeg,webp,', 'jpg', 'png', 'gif', 'svg']) && $image->isValid()) {
                $ext = $image->extension();
                $path = public_path($path);
                if ($ext == 'jpeg,webp,') $ext = 'jpg';
                $ext = 'webp';
                do {
                    $name = file_name(18, $ext);
                } while (file_exists($path . $name));
