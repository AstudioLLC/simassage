...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.question.show', ['id' => ':slug']) }}';

        const showGallery = '{!! __('app.Gallery') !!}';
        const addGallery = '{{ route('admin.gallery.index', ['gallery' => 'question', 'key' => ':slug']) }}';
        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.question.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.question.listPortion') }}',
                type: 'POST',
                // data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.question.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.question.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
