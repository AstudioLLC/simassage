...
<form action="{!! $edit ? route('admin.job.edit', ['id' => $item->id]) : route('admin.job.store') !!}"
...
{{--            </div>--}}
            <div class="form-group border-bottom">
                @formTitle(['title' => 'Salary'])@endformTitle
                <div class="card-body">
                    <input type="text"
                           name="salary"
                           class="form-control form-control-sm form-control-alternative{{ $errors->has('salary') ? ' is-invalid' : '' }}"
                           id="salary"
                           placeholder="Salary"
                           value="{{ old('salary', $item->salary ?? null) }}">
