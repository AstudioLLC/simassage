...
@section('titleSuffix')
    |
    <a href="{!! route('admin.delivery_regions.add') !!}" class="text-cyan">
            ...
            <div class="table-responsive p-2">
                <table class="table table-striped m-b-0 columns-middle init-dataTable">
                    ...
                    <tr>
                        ...
                        <th>{{ t('Admin pages list.settlements') }}</th>
                        <th>{{ t('Admin pages list.action') }}</th>
                    </tr>
                    </thead>
                    <tbody class="table-sortable" data-action="{{ route('admin.delivery_regions.sort') }}">
                        @foreach($items as $item)
                            <tr class="item-row" data-id="{!! $item->id !!}">
                                <td class="item-title">{{ $item->title}}</td>
                                <td>
                                    <a href="{{ route('admin.delivery_cities.main', ['id' => $item->id]) }}">
                                        {{ t('Admin pages list.Go to settlements') }} ({!! $item->cities_count !!})
                                    </a>
                                </td>
                                <td>
                                    <a href="{{ route('admin.delivery_regions.edit', ['id' => $item->id]) }}"
                                       {!! tooltip(t('Admin action buttons.edit')) !!} class="icon-btn edit">
