...
@section('content')
    <div class="col-12">
        <div class="card p-3">
            <form method="post" action="{{route('admin.users.packages.submit',['id'=>$company->id])}}" class="d-flex flex-column">
                @csrf
                ...
                <select class="js-example-basic-multiple" name="package_id"  id="package_id">
                    @foreach($packages as $package)
                            <option value="{{$package->id}}" {{(in_array($package->id,$company_package)?'selected':null)}}> {{$package->title}} </option>
                    @endforeach
                </select>
