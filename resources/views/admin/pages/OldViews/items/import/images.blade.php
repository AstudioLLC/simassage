...
@section('title', t('Admin items.import zip'))
@section('content')
            ...
            <div class="col-12 col-lg-6">
                <div class="card">
                    <div class="c-body">
                        @if(!empty($errors) && $errors->has('zip'))
                            <p class="text-red">
                                {{ t('Admin zip import.choose zip file') }}
                            </p>
                        @endif
                        <div class="mt-4">
                            @file(['name'=>'file', 'title'=>t('Admin zip import.choose zip file')])@endfile
                        </div>
                        <div class="pt-2 text-right">
                            <button type="submit" id="form-submit" class="btn btn-success">
                                {{ t('Admin excel import.import') }}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                @if(!empty(old('count')))
                    @if(old('changed_count') || old('changed_count')==0 )
                        <p class="text-greens">
                            {{ t('Admin zip import.success elements') }} <strong class="text-black">{{ old('changed_count') }}</strong>
                        </p>
                        <p class="text-red">
                            {{ t('Admin zip import.failed elements') }} <strong class="text-black"> {{ old('count') - old('changed_count') }}</strong>
                        </p>
                    @endif
                @endif
            </div>
