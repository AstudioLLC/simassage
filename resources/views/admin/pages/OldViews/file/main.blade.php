...
@section('titleSuffix')
    |
    <a href="{!! route('admin.file.add', ['file' => $file, 'key' => $key]) !!}" class="text-cyan">
        ...
        <div class="card">
            <div class="table-responsive p-2">
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    <tbody class="table-sortable" data-action="{{ route('admin.file.sort') }}">
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            <td class="item-title">{{ $item->title}}</td>
                            <td class="text-right">
                                <a href="{{ route('admin.file.edit', ['file' => $file, 'key' => $key, 'id' => $item->id]) }}"
