...
@push('js')
    ...
    <script>
        ...
        const galleryUrl = '{{ route('admin.gallery.index', ['gallery' => 'prices', 'key' => ':slug']) }}';

        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.prices.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            columns: [
                ...
                },
                {
                        ...
                        render: function(data, type, row) {
                            // Check if the rendering is for display, if not, return the raw data
                            if (type !== 'display') {
                                return data;
                            }

                            // Set the maximum character limit
                            var charLimit = 50; // Change this to your desired limit

                            // Check if the data length is greater than the character limit
                            if (data.length > charLimit) {
                                return data.substring(0, charLimit) + '...';
                            }
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.prices.listPortion') }}',
                type: 'POST',
                /*data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.prices.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.prices.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
