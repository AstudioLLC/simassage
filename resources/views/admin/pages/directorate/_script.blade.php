...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.directorate.show', ['id' => ':slug']) }}';

        const showGallery = '{!! __('app.Gallery') !!}';
        const addGallery = '{{ route('admin.gallery.index', ['gallery' => 'directorate', 'key' => ':slug']) }}';
        const galleryVideoTooltip = '{{ __('app.Video gallery') }}';
        const galleryVideoUrl = '{{ route('admin.videos.index', ['video'=>'directorate', 'key'=>':slug']) }}';
        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.directorate.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.directorate.listPortion') }}',
                type: 'POST',
                // data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.directorate.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.directorate.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
