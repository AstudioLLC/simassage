...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.applyJobs.show', ['id' => ':slug']) }}';

        const showGallery = '{!! __('app.Gallery') !!}';
        const addGallery = '{{ route('admin.gallery.index', ['gallery' => 'library', 'key' => ':slug']) }}';
        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.applyJobs.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.applyJobs.listPortion') }}',
                type: 'POST',
                // data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.applyJobs.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.applyJobs.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
