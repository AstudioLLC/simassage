...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.doctors.show', ['id' => ':slug']) }}';

        const showGallery = '{!! __('app.Gallery') !!}';
        const addGallery = '{{ route('admin.gallery.index', ['gallery' => 'members', 'key' => ':slug']) }}';
        const galleryVideoTooltip = '{{ __('app.Video gallery') }}';
        const galleryVideoUrl = '{{ route('admin.videos.index', ['video'=>'members', 'key'=>':slug']) }}';
        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.doctors.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
                ...
                },
                {
                    ...
                    render: function (id, q, row) {
                        ...
                        tools += '<a class="btn btn-sm btn-icon-only btn-outline-default" ' +
                            'href="' + galleryVideoUrl.replace(':slug', row.id) + '" ' +
                            'title="' + galleryVideoTooltip +' ('+row.videos_count+')">' +
                            '<i class="far fa-file-video"></i>' +
                            '</a>';
                        tools += '<a class="btn btn-sm btn-icon-only btn-outline-info" ' +
                            'href="' + addGallery.replace(':slug', row.id) + '" ' +
                            'title="' + showGallery +'">' +
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.doctors.listPortion') }}',
                type: 'POST',
                // data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.doctors.active', ['id' => ':slug']) !!}";
        ...
        $(document).on('click', '.show-home-changer span', function() {
            ...
            let url = "{!! route('admin.doctors.show_home', ['id' => ':slug']) !!}";
            var thisItemId = $(this).parents('tr').data('id');
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.doctors.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
