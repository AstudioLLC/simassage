...
@push('js')
    ...
    <script>
        ...
        const showUrl = '{{ route('admin.directorate2.show', ['id' => ':slug']) }}';

        const showGallery = '{!! __('app.Gallery') !!}';
        const addGallery = '{{ route('admin.gallery.index', ['gallery' => 'directorate2', 'key' => ':slug']) }}';
        const galleryVideoTooltip = '{{ __('app.Video gallery') }}';
        const galleryVideoUrl = '{{ route('admin.videos.index', ['video'=>'directorate2', 'key'=>':slug']) }}';
        const editTooltip = '{!! __('app.Edit') !!}';
        const editUrl = '{{ route('admin.directorate2.edit', ['id' => ':slug']) }}';
        ...
        $('.dataTable').dataTable({
            ...
            //order: [],
            ajax: {
                url: '{{ route('admin.directorate2.listPortion') }}',
                type: 'POST',
                // data: function (e) {
        ...
        $(document).on('click', '.active-changer span', function() {
            ...
            let url = "{!! route('admin.directorate2.active', ['id' => ':slug']) !!}";
        ...
        $('#itemDeleteForm').on('submit', function () {
            let url = "{!! route('admin.directorate2.delete', ['id' => ':slug']) !!}";
            if (blocked) return false;
            blocked = true;
