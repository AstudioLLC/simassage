    ...
    <div class="container-fluid mt--6">
        <div class="card">
            ...
            <div class="card-body border-0">
                <form action="{!! route('admin.departments.personnel.add',['id'=>$parentId]) !!}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('post')
                    @if ($errors->any())
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-12">
                            <div class="form-group border-bottom">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('title.'.$iso, tr($departmentPersonnel, 'title', $iso)) }}">
                                @if ($errors->has('title'))
                        ...
                        <strong>{{ $errors->first('title') }}</strong>
                            ...
                            <div class="form-group border-bottom">
                                @formTitle(['title' => 'Doctors'])@endformTitle
                                <div class="card-body">
                                    <label for="filters_ids">Choose doctors</label>
                                    <select class="js-example-basic-multiple" style="width: 100%" name="members_ids[]"
                                            id="service_ids" multiple="multiple">
                                        @foreach($personnel as $person)
                                            <option value="{{$person->id}}"
                                                {{ (in_array($person->id,$departmentsCategory))?'selected':'null'}}
                                            > {{$person->title}} </option>
                                        @endforeach
                                    </select>
