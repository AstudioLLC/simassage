    ...
    <div class="container-fluid mt--6">
        <div class="card">
            ...
            <div class="card-body border-0">
                <form action="{!! route('admin.departments.price.add',['id'=>$parentId]) !!}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('post')
                    @if ($errors->any())
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-12">
                            <div class="form-group border-bottom">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('title.'.$iso, tr($departmentPrice, 'title', $iso)) }}">
                                @if ($errors->has('title'))
                        ...
                        <strong>{{ $errors->first('title') }}</strong>
                            ...
                            <div class="form-group border-bottom">
                                @formTitle(['title' => 'Prices'])@endformTitle
                                <div class="card-body">
                                    <label for="filters_ids">Choose services</label>
                                    <select class="js-example-basic-multiple" style="width: 100%" name="price_ids[]"
                                            id="price_ids" multiple="multiple">
                                        @foreach($prices as $price)
                                            <option value="{{$price->id}}"
                                                {{ (in_array($price->id,$departmentsCategory))?'selected':'null'}}
                                            > {{$price->title}} </option>
                                        @endforeach
                                    </select>
