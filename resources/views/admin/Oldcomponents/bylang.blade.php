...
@foreach($languages as $language)
    ...
    @section('bylang_content')
        <div class="tab-pane{!! $active !!} {{ $tp_classes ?? 'normal-p' }} " id="{!! $id.$prefix !!}" role="tabpanel">
            {!! ${'bylang'.$prefix} !!}
        </div>
    @if($loop->first) @overwrite @else @append @endif
@endforeach
<div id="{{ $id }}" class="bylang">
    <div class="bylang-header {{ $class ?? '' }}">
        <div class="bylang-title {!! !empty($title)?'has-title':null; !!}">{{ $title ?? '' }}</div>
