...
Route::prefix('queuing_message')->name('queuing_message.')->group(function (Router $router) {
    $controller = 'QueuingMessagesController@';

    $router->get('list', $controller . 'index')->name('index');
    $router->post('listPortion', $controller . 'listPortion')->name('listPortion');

    $router->get('listStatus', $controller . 'listStatus')->name('listStatus');
    $router->post('updateStatus', $controller . 'updateStatus')->name('updateStatus');
